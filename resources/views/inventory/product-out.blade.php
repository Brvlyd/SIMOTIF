<!-- resources/views/inventory/product-out.blade.php -->
@extends('layouts.app')
...
<div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
    <div class="p-6 bg-white border-b border-gray-200">
        <h2 class="text-2xl font-semibold text-gray-900 mb-6">Record Product Out</h2>
        ...
        <form action="{{ route('inventory.product-out') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <select id="product_id" name="product_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    @foreach($products as $product)
                        <option value="{{ $product->id }}">
                            {{ $product->name }} - {{ $product->brand }} (Available: {{ $product->stock }})
                        </option>
                    @endforeach
                </select>
            </div>
